...
function renderTrendSvg(){
  ...
  if(!days.length){ wrap.innerHTML='<p style="color:#9CA3AF;font-size:13px;">Sem dados para exibir.</p>'; return; }
  var comp=days.map(function(k){return byDay[k].comp;});
  var leads=days.map(function(k){return byDay[k].leads;});
  var maxV=Math.max.apply(null, comp.concat(leads).concat([1]));
  var w=760, h=200, padL=30, padB=24, padT=10, padR=10;
  var innerW=w-padL-padR, innerH=h-padT-padB;
  function pt(i,v){ var x=padL+(days.length>1? (i/(days.length-1))*innerW : innerW/2); var y=padT+innerH-(v/maxV)*innerH; return x+','+y; }
  function pathFor(arr){ return arr.map(function(v,i){ return (i===0?'M':'L')+pt(i,v); }).join(' '); }
  var stepLabel=Math.max(1, Math.ceil(days.length/8));
  var labels=days.map(function(k,i){
    if(i%stepLabel!==0 && i!==days.length-1) return '';
    var x=padL+(days.length>1? (i/(days.length-1))*innerW : innerW/2);
    return '<text x="'+x+'" y="'+(h-6)+'" font-size="10" fill="#9CA3AF" text-anchor="middle">'+k.slice(8,10)+'/'+k.slice(5,7)+'</text>';
  }).join('');
  var svg='<svg viewBox="0 0 '+w+' '+h+'" style="width:100%;height:220px;">'
    +'<path d="'+pathFor(comp)+'" fill="none" stroke="#1F2937" stroke-width="2"/>'
    +'<path d="'+pathFor(leads)+'" fill="none" stroke="#E01A43" stroke-width="2"/>'
    +labels
    +'</svg>'
    +'<div style="display:flex;gap:16px;font-size:12px;color:var(--graphite-soft);margin-top:6px;">'
    +'<span><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:#1F2937;margin-right:5px;"></span>Completadas</span>'
    +'<span><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:#E01A43;margin-right:5px;"></span>Leads</span>'
    +'</div>';
  wrap.innerHTML=svg;
}
...
<div class="panel"><h3>Tendência diária</h3><div id="trendChart"></div></div>
...
async function exportInteractiveHtml() {
  ...
  downloadHtml(html, `relatorio-lopes-todas-equipes-protegido.html`);
}
